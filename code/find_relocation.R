#This is the code for finding the potential sites for relocation;
...
#find three more points for relocation;
target.n.sites <- 10
...
###################################################################
#which(V(sub.net)$name=="23060408001")
#Philip Lee: "23060416401" "23150136701" "23060322901"
k.site.ID="23060322901"
...
test <- cbind(dat.manhole$UNITID[sub.net.node.ind],cnt.up.nodes[sub.net.node.ind])
View(test[order(as.numeric(test[,2]),decreasing = T),])

#South Fulton: "13860408001" "23040109701" "23140107401"
k.site.ID="23140107401"
sub.net.node.ind <- sort(c(which(V(net)$name==k.site.ID), get_upstream_nodes(net,which(V(net)$name==k.site.ID))))
sub.net <- induced_subgraph(net,sub.net.node.ind)
