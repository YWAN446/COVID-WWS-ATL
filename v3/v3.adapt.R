...
n.week=4
target.n.sites <- 5
pdf(paste0("./v3/output/plots/update_map_",k.site.ID,"_",target.n.sites,".pdf"),width=30,height=30)
par(mfrow=c(3,3))
...
for (k.iter in 1:8){
  ...
  weight.decrease <- c(rep(9/10,n.sites.main),rep(3/4,n.sites.branch))
  weight.increase <- c(rep(10/9,n.sites.main),rep(4/3,n.sites.branch))
  ...
  sub.net.manholes$weight[which(sub.net.manholes$weight>(n.manhole/50))] <- n.manhole/100 
  sub.net.manholes$weight[which(sub.net.manholes$weight<(n.manhole/5000))] <- n.manhole/2500
